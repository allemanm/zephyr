...
int mcp980x_reg_read(const struct device *dev, uint8_t reg, uint16_t *val, uint8_t len)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to write read i2c (%d)", ret);
		return ret;
	}
...
int mcp980x_reg_write(const struct device *dev, uint8_t reg, uint16_t val, uint8_t len)
{
	...
	ret = i2c_write_dt(&cfg->i2c, buf, sizeof(reg) + len);
	if (ret < 0) {
...
static int mcp980x_set_temperature_resolution(const struct device *dev, uint8_t resolution)
{
	...
	ret = mcp980x_reg_write(dev, MCP980X_REG_CONFIG, config, sizeof(uint8_t));
	if (ret < 0) {
...
static int mcp980x_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	ret = mcp980x_reg_read(dev, MCP980X_REG_TEMP_AMB, &data->reg_val, sizeof(uint16_t));
	if (ret < 0) {
		LOG_ERR("Failed to read ambient temperature (%d)", ret);
		return ret;
	}
			       ...
			       struct sensor_value *val)
{
	...
	val->val1 = (int32_t)(temp / MCP980X_TEMP_SCALE_CEL);
	temp -= val->val1 * MCP980X_TEMP_SCALE_CEL;
	val->val2 = (int32_t)(temp * 1000000) / MCP980X_TEMP_SCALE_CEL;
...
int mcp980x_init(const struct device *dev)
{
	...
	if (gpio_is_ready_dt(&cfg->int_gpio)) {
		...
		if (ret < 0) {
			LOG_ERR("Failed to setup interrupt (%d)", ret);
		}
	}
#endif /* CONFIG_MCP980X_TRIGGER */
